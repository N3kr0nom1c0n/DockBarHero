...
- Goal: integrate the verified lore manga reader with the newer Heroes, Class Actions, Loot, farming-status, and production-sprite work.
- Current checkpoint: `codex/lore-manga-vertical-slice` contains the Book implementation and fixed-spread repair plus a verified merge of `feature/class-actions-and-loot`.
...
- Remaining: user review plus the still-pending compact, audio, VoiceOver, light-appearance, reduced-motion, potentiometer, and reaction-bubble interaction checks.
...
- Lore: validated page/dialogue sidecars, frontier unlock resolver, Book-scoped opt-in TTS, four-frame sheets, fixed high-contrast pages, responsive right-to-left spreads, and anchored reactions.
...
- Parent orchestrator: keep the verified combined Book open for user review and record any requested follow-up.
...
- Combined checkpoint: 400 arm64 tests and 15 sprite-pipeline tests passed with zero failures; clean unsigned build succeeded.
- Exact combined worktree bundle launched with `--open-book` as the only DockBarHero process (PID 39103).
- Live inspection confirmed the 1100-point initial window, fixed opaque captions, wheel-stable layout, and right-to-left two-page navigation; unexercised items remain pending in the lore review packet.
